...
public class HLReplaceTokenProvider implements ReplaceTokenProvider, ActionConvertor {
    ...
    public HLReplaceTokenProvider(Project project) {
    ...
    public Map<String,String> createReplacements(String action, Lookup lookup) {
        ...
        if (isHadraFile(fo)) {
            for (SourceGroup group : ProjectUtils.getSources(project).getSourceGroups(HLSourcesImpl.TYPE_HADRA)) {
                ...
                if (relPath != null) {
                    ...
                    replaceMap.put(CLASSPATHSCOPE, group.getName().equals(HLSourcesImpl.NAME_HADRA_TESTSOURCE) ? "test" : "runtime"); //NOI18N
                    return replaceMap;
                }
            }
        }
    ...
    public String convert(String action, Lookup lookup) {
        if (ActionProvider.COMMAND_RUN_SINGLE.equals(action) ||
            ...
            if (isHadraFile(fo)) {
                ...
                //TODO this only applies to hadra files with main() method.
                // we should have a way to execute any hadra script? how?
                // running hadra tests is another specialized usecase.
    ...
    private boolean isInTestFolder(FileObject file) {
        if (file.getPath().contains("/test/hadra")) { //NOI18N
            return true;
        }
        return false;
    }

    private boolean isHadraFile(FileObject file) {
        if (file != null && "text/x-hl".equals(file.getMIMEType())) { //NOI18N
            return true;
        }
        return false;
    }
