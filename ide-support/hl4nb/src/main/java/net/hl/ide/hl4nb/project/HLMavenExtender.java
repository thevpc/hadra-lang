...
//    }
//)
public class HLMavenExtender /*implements GroovyExtenderImplementation*/ {

    private final FileObject pom;

    
    public HLMavenExtender(Project project) {
                ...
                public void run() throws IOException {
                    Utilities.performPOMModelOperations(pom, Collections.singletonList(new ModelOperation<POMModel>() {
                        ...
                        public void performOperation(POMModel model) {
                            if (MvnUtils.hasModelDependency(model, MvnUtils.HADRA)) {
                                retValue[0] = true;
                            } else {
                                retValue[0] = false;
                            }
        ...
        try {
            pom.getFileSystem().runAtomicAction(new FileSystem.AtomicAction() {
                ...
                public void run() throws IOException {
                    ...
                    operations.add(new HLAddDependency());
                    operations.add(new HLAddMavenCompilerPlugin());
